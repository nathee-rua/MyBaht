...
import { ChevronLeft, MessageCircle, Sparkles, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function LineSettingsPage() {
  ...
  const handleUnlink = async () => {
    if (!confirm('Are you sure you want to unlink your LINE account?')) return;
  ...
  return (
    <div className="page-container px-4 pt-4 flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3 pb-2 border-b border-border/20">
        ...
        <h1 className="text-lg font-bold text-text-primary">{t('line.connect')}</h1>
      </div>
      ...
      <div className="card-base p-6 flex flex-col items-center text-center gap-4">
        ...
        <div>
          <h2 className="text-base font-bold text-text-primary">
            {linked ? t('line.linked') : t('line.unlinked')}
          </h2>
          <p className="text-xs text-text-muted mt-1 leading-relaxed">
            {linked
              ? 'Your account is linked to LINE! Send receipt photos or alerts to process them automatically.'
              : 'Link your LINE account to upload slips and receipts on the go.'}
          </p>
        </div>
        ...
        {linked && (
          ...
          >
            {unlinking ? 'Unlinking...' : 'Disconnect LINE Account'}
          </button>
        )}
      </div>
        ...
        <div className="flex flex-col gap-4 animate-scale-in">
          ...
          <div className="card-base p-5 flex flex-col gap-4">
            ...
            <p className="text-xs text-text-secondary leading-relaxed">
              1. Open LINE, search for the official bot account or scan the QR code.
              <br />
          ...
          <div className="flex gap-3 p-4 bg-secondary/10 border border-border/20 rounded-2xl text-[11px] text-text-secondary leading-relaxed">
            ...
            <p>
              Linking your LINE account grants the bot permission to parse your receipt images and create expense entries. The bot only reads messages sent directly to it.
            </p>
          </div>
        </div>
      )}
